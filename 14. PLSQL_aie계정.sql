/*
    ...
    PL/SQL 구조
        ...
        - [예외처리부( EXCEPTION SECTION )] : EXCEPTION으로 시작
                    예와발생시 해결하기 위한 구문을 미리 기술해둘 수 있는 부분
*/
-- 화면에 오라클 출력( 껏다 다시키면 반드시 실행해야함 )
SET SERVEROUTPUT ON;
...
BEGIN
    ...
    -- DBMS_OUTPUT.PUT_LINE( '보너스 : ' || NVL( E.BONUS, '없음' )); 타입이 안맞아서 오류
...
DECLARE
    E EMPLOYEE%ROWTYPE;
BEGIN
    SELECT EMP_NAME, SALARY, BONUS --무조건 *이걸 사용해야함. 필요한거만 가져올 수 없음
        INTO E
    FROM EMPLOYEE
    WHERE EMP_ID = &사번;
...
--단, 보너스를 받지않는 사원은 보너스율 출력전에 '보너스를 지급받지 않는 사원입니다' 출력
DECLARE
    EID EMPLOYEE.EMP_ID%TYPE;
    ENAME EMPLOYEE.EMP_NAME%TYPE;
    SAL EMPLOYEE.SALARY%TYPE;
...
/*
    ...
    실행 : NCODE값이 KO일 경우 => TEAM변수에 '국내팀'
              NCODE KO가 아니면  => TEAM변수에 '해외팀'
...
/*
    3. 예외처리부
        ...
        *시스템 예외(오라클에서 미리 정의해둔 예외)
            ...
            - DUP_VAL_ON_INDEX : UNIQUE 제약조건 위배되었을 경우
            .....등등
*/
-- 사용자가 입력한 수로 나눗셈 연산
DECLARE
...
BEGIN
    UPDATE EMPLOYEE
        ...
        WHERE EMP_NAME = '김정보';
    EXCEPTION
        WHEN DUP_VAL_ON_INDEX THEN DBMS_OUTPUT.PUT_LINE( '이미 존재하는 사번 입니다.' );
END;
/

--사수 200번은 6명, 201 하나도 없고, 222 1명
DECLARE
    EID EMPLOYEE.EMP_ID%TYPE;
    ENAME EMPLOYEE.EMP_NAME%TYPE;
BEGIN
