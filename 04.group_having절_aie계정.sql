/*
    <GROUP BY 절>
    그룹을 기준으로 제시할 수 있는 구문(여러 그룹기준별로 여러 그룹으로 묶을 수 있음)
    여러개의 값들을 하나의 그룹으로 묶어서 처리할 목적으로 사용
*/
SELECT SUM(SALARY)
FROM EMPLOYEE; --전체사원을 하나의 그룹으로 묶어서 총합을 구한것.
...
--각 직급별 사원수, 급여 합계 조회. 직급별 내림차순으로 조회
...
FROM EMPLOYEE
GROUP BY JOB_CODE
ORDER BY 4; --4는 컬럼의 번호(SUM(SALARY))
...
/*
    <HAVING 절>
    그룹에 대한 조건을 제시할 때 사용되는 구문(주로 그룹함수식을 가지고 조건을 제시할 때 사용. WHERE가 아니라 HAVING을 사용함)
    WHERE절을 쓰면 그룹함수는 허가되지않음
*/
...
/*
    ...
    - CUBE(컬럼1, 컬럼2) : 컬럼1을 가지고 다시 중간 집계를 내고, 컬럼2를 가지고 다시 중간 집계를 내는 함수
    컬럼이 하나일때에는 할 필요가 없음
*/
--각 직급별 급여합
...
--CUBE, ROLLUP의 차이점을 보려면 컬럼 2개는 있어야 가능
--컬럼이 2개일 때
--ROLLUP(컬럼1, 컬럼2)
SELECT DEPT_CODE, JOB_CODE, SUM(SALARY)
FROM EMPLOYEE
...
--주의사항
--집합연산자를 사용할 경우, 컬럼의 개수와 컬럼명이 동일해야한다.


------------------------------------------------- 3. UNION ALL -------------------------------------------------
--부서코드가 D5인 사원 이면서 급여가 300만원 초과인 사원을 조회(중복값도 모두 출력)
SELECT EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5'
UNION ALL
...
------------------------------------------------- 4. MINUS -------------------------------------------------
--직급코드가 'D5'인 사원들중 급여가 300만원 초과인 사원을 제외한 사원 조회
