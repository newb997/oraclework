/*
    ...
    * 변경할 내용 
        1) 컬럼 추가 / 수정 / 삭제
        2) 제약조건 추가 / 삭제 -> 수정불가( 수정하고자하면 삭제 후 새로 추가 )
        3) 컬럼명 / 제약조건명 / 테이블명 변경
*/
...
--DEPT_COPY테이블에 LNAME 컬럼 추가, DEFAULT(기본값) 한국으로 추가
...
--DEPT_COPY 테이블의 DEPT_ID의 데이터 타입을 NUMBER로 수정
ALTER TABLE DEPT_COPY MODIFY DEPT_ID NUMBER;    --오류발생 : 컬럼값에 영문이 있음, 또한 컬럼의 데이터 타입을 변경하기 위해서는 해당 컬럼의 값을
...
ALTER TABLE DEPT_COPY2 DROP COLUMN DEPT_ID;         --오류 : 최소 한 개의 컬럼은 존재해야 한다.
----------------------------------------------------------------------------------------------------------------------------------------------
...
/*
    ...
    만약 삭제하고 싶다면,
    방법 1 자식테이블을 먼저 삭제 후 부모테이블 삭제
    방법 2 그냥 부모테이블만 삭제하는데 제약조건까지 같이 삭제
