...
-- EMPLOYEE 테이블에서 새로운 행이 INSERT 될때마다자동으로 메시지 출력되는 트리거 정의
SET SERVEROUT ON;
...
COMMIT; --커밋해줘야 테이블에 들어가서 쓸 수 있음

--3. 상품 입고 테이블(TB_RPOSTOCK)
...
--TB_PRODUCT 테이블에 매번 자동으로 재고수량이 UPDATE되는 트리거를 정의

--TB_PROSTOCK 입고테이블에 INSERT이벤트 발생시
/*
       UPDATE TB_PRODUCT 
            SET STOCK_QUANT = STOCK_QUANT +현재 입고된 수량(INSERT된 자료의 STOCK_COUNT값)
...
--201번 상품 5개입고
INSERT INTO TB_PROSTOCK
VALUES( SEQ_SCODE.NEXTVAL, 201, SYSDATE, 5, 1700000);

--200번 상품 7개입고
INSERT INTO TB_PROSTOCK
VALUES( SEQ_SCODE.NEXTVAL, 200, SYSDATE, 7, 2000000);
...
INSERT INTO TB_PROSALE
VALUES( SEQ_SCODE.NEXTVAL, 202, SYSDATE, 15, 1200000);

INSERT INTO TB_PROSALE
VALUES( SEQ_SCODE.NEXTVAL, 201, SYSDATE, 3, 2000000);


--출고시 재고수량이 부족할 경우 출고가 안되게
/*
    사용자 함수 예외처리
        RAISE_APPLICATION_ERROR([ 에러코드 ],[ 에러메시지 ])
        - 에러코드 : -20000 ~ -20999사이의 코드
*/
CREATE OR REPLACE TRIGGER TRG_SALE
